        ...
        <div class="span12">
            ...
            <div class="widget-box">
                ...
                <div class="widget-title">
                    ...
                    <h5></h5>
                    <form id="mainForm" method="post" action="${ctx}/smsaudit/yzmaudit"
                        ...
                        <div class="search">
                            ...
                            <ul class="">
                                ...
                                <li style="float: right">
                                    ...
                                    <input type="button" value="重置" onclick="toReset()" style="background: #ff9600;"/>
                                    <input type="button" value="转审" onclick="transferAuditBatch()"/>
                                    <input type="button" value="批量审核通过" onclick="updateStatusBatch(1)"/>
                                    <input type="button" value="批量审核不通过" onclick="updateStatusBatch(2)"/>
                                </li>
                            </ul>
                        </div>
                    </form>
                                        ...
                                        <c:if test="${(smstype eq 0) or (smstype eq 4) or (smstype eq 5)}">
                                            <br/> <span href="javascript:;" style="cursor:pointer;"
                                                    onclick="addTemplate('${clientid}', '<c:out
                                                            value="${sign}"/>', '<c:out
                                                            value="${content}"/>', '${smstype}',3311)">添加智能模板</span><br>
                                        </c:if>
                                    </u:authority>
                                </td>
                                <td>
...
<script type="text/javascript">
    ...
    function releaseSmsAudit() {
        $.ajax({
            type: "post",
            url: "${ctx}/smsaudit/updateYZMStatus",
            async: false,
            data: {
...
// 			}
    ...
    function showAuditNum() {
        var text1 = "待审核记录数(" + yzmNum + ")";
        var text2 = "审核待发送数(" + yzmNumSending + ")";
        var text3 = "被锁定审核数(" + yzmLockNum + ")";
        $("#auditNum").text(text1);
        $("#sendNum").text(text2);
        $("#lockNum").text(text3);
    ...
    function getAuditNum() {
        $.ajax({
            ...
            success: function (data) {
                ...
                var lockNum = 0;
                if (data != null) {
                    auditNum = data.yzmNum;
                    sendNum = data.yzmNumSending;
    ...
    function addTemplate(clientId, sign, content, smsType,menuId) {
        var url = '${ctx}/smsaudit/template/add?clientId=' + clientId + "&smsType=" + smsType
            + "&sign=" + sign + "&content=" + content + "&menuId=" + 3311;
        url = encodeURI(url);
