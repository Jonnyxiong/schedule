				...
				<div class="widget-box">
					<div class="widget-title">
						...
						<%-- <u:authority menuId="236">
							<span class="label label-info">
								<a href="javascript:;" onclick="createQueueOnMiddleware('${data.middlewareId}', '${data.middlewareName}')">创建以下队列</a>
							</span>
						</u:authority> --%>
					...
					<div class="widget-content nopadding">
						<table class="table table-bordered table-striped">
							...
							<tbody>
								<s:iterator value="data.page.list" status='st' var="i">
									<tr>
										<td>${st.index + 1}</td>
										<td>${i.middlewareId}</td>
										<td>${i.middlewareName}</td>
										<td>${i.queue}</td>
										<td>${i.exchange}</td>
										<td>${i.routingKey}</td>
